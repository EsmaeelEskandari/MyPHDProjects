...
float* ZInvEstFromW::Analysis( TString MT2_REGIME,  bool IS_MC, TString outputfile, int MIN_NJETS, int NELE, int NMU, float MT2CUT_LOW, float MT2CUT_HIGH,  TString trigger, TString filter,  bool BENRICH, bool CORRECT_EFF, float * INPUT){
  ...
  //Efficiencies
//  TString EFF_DIR = "/shome/leo/Analysis/Efficiencies/"; comm 21 jul 2013
  TString EFF_DIR = "/dataLOCAL/MT2Top/Efficiencies/";
 ...
 /* else if(tsname=="HT-Data") {tsname="data";fSamples[i].sname="data";}
    ...
    else if(tsname=="MJ-Data") {tsname="data";fSamples[i].sname="data";}
    else if( fSamples[i].name == "DYToNuNu_HT_100To200" ||
	     fSamples[i].name == "DYToNuNu_HT_200To400" ||
	     fSamples[i].name == "DYToNuNu_HT_400" ) {tsname="Zinv";fSamples[i].sname="Zinv";}
 ...
 //   if(tsname=="HT-Data") tsname="data";
    ...
    while(myEvtList->GetEntry(counter++) !=-1){
      ...
      //Weights
      Double_t weight = 1;
     if (!fMT2tree->misc.isData && BENRICH == false) weight = sample_weight * fMT2tree->pileUp.Weight * fMT2tree->SFWeight.BTagCSVM40eq0; // pile-up reweighting for MC
    else if(!fMT2tree->misc.isData && BENRICH == true  ) weight = sample_weight * fMT2tree->pileUp.Weight * fMT2tree->SFWeight.BTagCSV40ge1;
...
//    float minDPhi = fMT2tree->misc.MinMetJetDPhi;
      float minDPhi = fMT2tree->misc.MinMetJetDPhi4 ; //fMT2tree->misc.MinMetJetDPhi;  
	...
	if(fMT2tree->jet[j].lv.Pt() > JET_PT) NJets++;
	  ...
	  if( fMT2tree->jet[jet].bTagProbCSV > 0.679  ) isBTagged=true;
        }
        else{
       // if( fMT2tree->jet[jet].bTagProbSSVHP > 2. )  isBTagged=true;
          if( fMT2tree->jet[jet].bTagProbCSV > 0.898  )  isBTagged=true;
        }
   ...
   //	  if( fMT2tree->jet[jet].isPATPFIDLoose!=1) continue;
	  if( fMT2tree->jet[jet].isPFIDLoose!=1) continue;	
	...
	minDPhi = fMT2tree->misc.MinMetJetDPhi4 ;	
...
*/
	if(isWELE and ELEorMU=="ELE") histos["MT2_LeptAcc_"+tsname+"_"+ mysel]->Fill(MT2,weight);
	else if  (isWMU and ELEorMU=="MU") histos["MT2_LeptAcc_"+tsname+"_"+mysel]->Fill(MT2,weight);
      }
 ...
 //     if(ELEorMU == "MU"){ if( NEles!=0 ) weight=0; }
     if(ELEorMU == "MU" /*|| ELEorMU == "ELE"*/){ if(! (fMT2tree->NEles == 0 || fMT2tree->ele[0].lv.Pt() < 5 /*|| fMT2tree->NMuons == 0 || fMT2tree->mu[0].lv.Pt() < 5*/)) weight=0; }
      else { if( NMuons!=0 ) weight=0; }
      counterH[mysel][tsname]->Fill(MT2,weight);
      if(type=="mc")  counterH[mysel]["MC"]->Fill(MT2,weight);
       

  //    double z_mc_pred_err;

    // if( (nBTags=1) && (tsname=="Zinv" ) ){ 
   //  float z_mc_pred = counterH["leptVetoSel"]["Zinv"]->IntegralAndError(1, counterH["leptVetoSel"]["Zinv"]  ->GetNbinsX()+1, z_mc_pred_err);
     //  }
 ...
 */
	...
	for(int b=0; b<nAllBTags; b++){
      ...
      if(weight!=0){
	histos["Lept_pt_"+tsname+"_"+mysel]->Fill(theLept.Pt(), weight);
	histos["Lept_eta_"+tsname+"_"+mysel]->Fill(theLept.Eta(), weight);
...
//      histos["caloHT_"+tsname+"_"+mysel]->Fill( fMT2tree->misc.caloHT50_ID, weight);
 ...
 cout<<"after get efficiency"<<endl;
  ...
  cout<<"after get efficiency 2"<<endl;
  cout << endl;
  ...
  //BTag scale factors
  float CSVT_scale =1.0; 
 //float CSVT_scale = 0.947;
  float CSVT_scale_err = sqrt(0.02*0.02 + 0.09*0.09);
  //float CSVT_scale_syst = 0.09;
  //  float CSVT_scale_err_perc = sqrt( CSVT_scale_err* CSVT_scale_err +CSVT_scale_syst*CSVT_scale_syst )/CSVT_scale;
  float CSVT_scale_err_perc = CSVT_scale_err/CSVT_scale;

  float CSVM_scale = 1.0;
//  float CSVM_scale = 0.963;

  float CSVM_scale_err = sqrt(0.01*0.01 + 0.10*0.10);
  //  float CSVM_scale_syst = 0.10;
  float CSVM_scale_err_perc = CSVM_scale_err /CSVM_scale;
  ...
  if(BENRICH){
    ...
    //TOP est computation
    if(!IS_MC) {
      mc_bkg *= CSVT_scale; //SF for MC bkg
      mc_bkg_err = mc_bkg;
      //mc_bkg_err = sqrt(  mc_bkg_err* mc_bkg_err/(mc_bkg*mc_bkg) + CSVT_scale_err_perc*CSVT_scale_err_perc ); //error is already 100%
     //mc_pred *= CSVT_scale;
      mc_pred_err = mc_pred*sqrt(  mc_pred_err* mc_pred_err/(mc_pred*mc_pred) + CSVT_scale_err_perc*CSVT_scale_err_perc );
    }
    ...
    if(est <= 0) {
      ...
      //BTag SF (data only)
      if(!IS_MC) { b_eff *=CSVT_scale; }
      //beff correction (from MC)
      est /= b_eff; 
      
      //errors
      est_syst_err += mc_bkg_err*mc_bkg_err; //MC bkg subtraction, 100% error
      if(!IS_MC) est_syst_err += est*est* CSVT_scale_err_perc* CSVT_scale_err_perc; // SF errors
      ...
      printf("|MC bkg subtraction, 100 percent error| %.3f |\n",mc_bkg_err);
      printf("|SF errors| %.3f |\n",est*CSVT_scale_err_perc);
      printf("|CSVT_scale_err = sqrt(0.02*0.02 + 0.09*0.09)| %.3f |\n",CSVT_scale_err);
      printf("|CSVT_scale_err_perc = CSVT_scale_err/CSVT_scale| %.3f |\n",CSVT_scale_err/CSVT_scale);
      printf("|MC errors on b-sel eff| %.3f |\n",est*b_eff_err_perc);
      printf("|est_syst_err| %.3f |\n", est_syst_err);
...
//    cout << "\n\n" << endl;
    ...
    if(MT2_REGIME=="MT2b"){
      ...
      result[5] = b_eff*CSVT_scale;
      result[6] = sqrt(b_eff_err_perc*b_eff_err_perc + CSVT_scale_err_perc*CSVT_scale_err_perc );
    }
  }
  ////////////////////////////////////////
  //ZInv estimate
...
//    getEfficiency( counterH["bSel"]["Top"], counterH["leptSel"]["Top"], &tt_bveto_eff, &tt_bveto_eff_err);
    getEfficiency( counterH["bSel"]["Top"], counterH["mtSel"]["Top"], &tt_bveto_eff, &tt_bveto_eff_err);
    tt_bveto_eff *= CSVM_scale;
    ...
    //WJets est
    if(!IS_MC) mc_bkg *= CSVM_scale;
    float w_est = data - mc_bkg - tt_bkg;
    
    //errors
    //float CSVM_scale_err_perc = sqrt(CSVM_scale_err*CSVM_scale_err + CSVM_scale_syst*CSVM_scale_syst) /  CSVM_scale; //SF
    ...
    if(!IS_MC) w_est_err_syst += w_est*w_est*CSVM_scale_err_perc*CSVM_scale_err_perc;
    ...
    cout<<endl;
      ...
      printf("|MC bkg subtraction, 100 percent error| %.3f |\n",mc_bkg_err);
      printf("|tt_bkg_err_syst| %.3f |\n",tt_bkg_err_syst);
      printf("|MC errors on b-veto eff| %.3f |\n",w_est*CSVM_scale_err_perc);
      printf("|w_est_err_syst| %.3f |\n", w_est_err_syst);
...
*/
    float z_est = w_est;
    float mc_factor = zw_ratio;
    mc_factor /= (wj_bveto_eff *CSVM_scale) ;
    //mc_factor /= lept_acc;
    //mc_factor /= tp_eff;
    mc_factor /= mt_eff;
    ...
    printf("|zw_ratio error  from MC| %.3f |\n",a);
    printf("|zw_ratio_pdf_err_perc| %.3f |\n",zw_ratio_pdf_err_perc);
    printf("|lept_acc_err| %.3f |\n", lept_acc_err);
    printf("|lept_acc_pdf_err| %.3f |\n",lept_acc*lept_acc_pdf_err_perc);
    printf("|tp_eff_err_perc = tp_eff_err/tp_eff| %.3f |\n", tp_eff_err_perc);
    printf("|MC error| %.3f |\n",mc_err);
    printf("|w_est_err_syst| %.3f |\n", w_est_err_syst);
    printf("| mt_eff_err_perc| %.3f |\n",  mt_eff_err_perc);
    printf("|b SF eff| %.3f |\n", INPUT[6]);
   ...
   double z_mc_pred_err;
   float z_mc_pred; 
   /*if (BENRICH){float*/ z_mc_pred = counterH["leptVetoSel"]["Zinv"]->IntegralAndError(1, counterH["leptVetoSel"]["Zinv"] ->GetNbinsX()+1, z_mc_pred_err);
...
*/
    printf( "| | *Top est (0b, 1l)* | *Top MC (0b, 1l)* | *WJets est (0b, 1l)* | *WJets MC (0b,1l)* | *ZInv est* | *ZInv MC* |\n" );
    printf( "| *MT2 [%.0f, %.0f)* | %.2f +- %.2f ( %.2f (stat) +- %.2f (syst) ) | %.2f +- %.2f |  %.2f +- %.2f ( %.2f (stat) +- %.2f (syst) ) |  %.2f +- %.2f |  %.2f +- %.2f ( %.2f (stat) +- %.2f (syst) ) | %.2f +- %.2f |\n", 
...
void ZInvEstFromW::createHistos(map<TString, TH1F*> *histos, vector<TString> samplesName ){
  ...
  for( vector<TString>::const_iterator s_i=samplesName.begin(); s_i != samplesName.end(); s_i++){
   ...
   hname="MT2_"+sname+"_bSel"; (*histos)[hname] = new TH1F(hname,";"+hname,15,100,600);
   // hname="MT2_"+sname+"_bSel"; (*histos)[hname] = new TH1F(hname,";"+hname,50,0,800);
    ...
    hname="MT_"+sname+"_bSel"; (*histos)[hname] = new TH1F(hname,";"+hname,15,0,200);
    ...
    //Leptons
    hname="Lept_pt_"+sname+"_leptSel"; (*histos)[hname] = new TH1F(hname,";"+hname,200,0,2000);
    hname="Lept_eta_"+sname+"_leptSel"; (*histos)[hname] = new TH1F(hname,";"+hname,100,-3,3);
 // hname="Lept_pt_bSel_"+sname; (*histos)[hname] = new TH1F(hname,";"+hname,200,0,2000);
    hname="Lept_pt_"+sname+"_bSel"; (*histos)[hname] = new TH1F(hname,";"+hname,10,0,300);
    hname="Lept_eta_"+sname+"_bSel"; (*histos)[hname] = new TH1F(hname,";"+hname,100,-3,3);
