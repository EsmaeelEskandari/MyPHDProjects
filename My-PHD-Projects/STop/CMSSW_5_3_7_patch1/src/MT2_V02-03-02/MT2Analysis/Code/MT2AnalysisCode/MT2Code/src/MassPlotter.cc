...
void MassPlotter::makeSmallCopy(int nevents, int sample) {
    ...
    for (int ii = 0; ii < fSamples.size(); ii++) {
        ...
        Int_t nentries = (Int_t) fSamples[ii].tree->GetEntries();
        ...
        for (Int_t i = 0; i < nentries; i++) {
            ...
            if (!(fMT2tree->misc.MET > 30 &&
                    ...
                    (fMT2tree->trigger.HLT_QuadJet80_v3 == 1) || (fMT2tree->trigger.HLT_QuadJet80_v4 == 1) ||
                    (fMT2tree->trigger.HLT_QuadJet80_v6 == 1)) &&
                    ((fMT2tree->NJetsIDLoose40 >= 4 && fMT2tree->jet[3].lv.Pt() >= 100.0) ||
                    (fMT2tree->NJetsIDLoose40 >= 5 && fMT2tree->jet[4].lv.Pt() >= 85.0))) ||

                    (((fMT2tree->trigger.HLT_SixJet45_v1 == 1) || (fMT2tree->trigger.HLT_SixJet45_v2 == 1) ||
                    (fMT2tree->trigger.HLT_SixJet45_v3 == 1) || (fMT2tree->trigger.HLT_SixJet45_v4 == 1) ||
                    (fMT2tree->trigger.HLT_SixJet45_v6 == 1)) &&
                    ((fMT2tree->NJetsIDLoose40 >= 6 && fMT2tree->jet[5].lv.Pt() >= 65.0) ||
                    (fMT2tree->NJetsIDLoose40 >= 7 && fMT2tree->jet[6].lv.Pt() >= 55.0))))))))
                continue;
...
void MassPlotter::PrintWEfficiency(int sample_index, TString process, std::string lept, Long64_t nevents, bool includeTaus) {
    ...
    enum counters_t {
        count_begin, all = count_begin, presel, NJetsIDLoose, PassJetID, MinMetJetDPhi, VectorSumPt, MT2, count_end
    };
    ...
    if (lept == "ele" && process == "W") {
        to_measure = "W->enu  recoed";
    } else if (lept == "muo" && process == "W") {
        to_measure = "W->munu recoed";
    } else if (lept == "ele" && process == "Top") {
        to_measure = "Top W->enu  recoed";
    } else if (lept == "muo" && process == "Top") {
        to_measure = "Top W->munu  recoed";
    }
    ...
    for (Long64_t jentry = 0; jentry < min(nentries, nevents); jentry++) {
        nb = Sample.tree->GetEntry(jentry);
        nbytes += nb;
        Sample.tree->SetBranchAddress("MT2tree", &fMT2tree);
...
void MassPlotter::PrintCutFlow(int njets, int nleps, TString trigger, TString cuts) {
    ...
    for (size_t i = 0; i < fSamples.size(); ++i) {
        ...
        while (myEvtList->GetEntry(counter++) != -1) {
            ...
            nb = fSamples[i].tree->GetEntry(jentry);
            nbytes += nb;
            fSamples[i].tree->SetBranchAddress("MT2tree", &fMT2tree);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (RemoveLepts && (Samples[i].sname == "DYToLL" || Samples[i].sname == "QCD_2")) {
            ...
            if (iter != RemoveLeptMap.end()) {
                ...
                cout << "found RemoveLepts==true, but no corresponding map" << endl;
                variable = var;
            }
        } else {
            variable = var;
...
void MassPlotter::MakePlot(std::vector<sample> Samples, TString var, TString maincuts, TString basecuts, int njets, int nbjets, int nleps, TString HLT,
    ...
    for (size_t i = 0; i < Samples.size(); ++i) {
        ...
        if (Samples[i].type == "susy") selection = TString::Format("(%.15f) * (%s)", weight, theCuts.Data()); //saeid
        //	TString selection;
...
void MassPlotter::TEST_Counting_MT2(TString basecut) {
    ...
    // === B O O K I N G ===
    TH2D *h_TN_MLSP_MSTop = new TH2D("h_TN_MLSP_MSTop", "", 100, 0, 2500, 100, 0, 2500);
    h_TN_MLSP_MSTop->Sumw2();
    TH2D *h_PN_MLSP_MSTop = new TH2D("h_PN_MLSP_MSTop", "", 100, 0, 2500, 100, 0, 2500);
    h_PN_MLSP_MSTop->Sumw2();
    TH2D *h_Eff_MLSP_MSTop = new TH2D("h_Eff_MLSP_MSTop", "", 100, 0, 2500, 100, 0, 2500);
    h_Eff_MLSP_MSTop->Sumw2();


    TH1D *h_PMT2_Bkg = new TH1D("h_PMT2_Bkg", "", 1, 0, 1);
    h_PMT2_Bkg->Sumw2();
    ...
    for (size_t i = 0; i < fSamples.size(); ++i) {
        ...
        //af: This line is put here to take PU_avg_weight into account.
        if (fPUReweight) weight = weight / (fSamples[i].PU_avg_weight);


        //        TString weights = ((fSamples[i].type != "data" || fSamples[i].type != "susy") ?
        //                TString::Format("(%.15f*pileUp.Weight)", weight) : TString::Format("(1.0)"));


        if (fSamples[i].type == "susy") {
            TString weights = TString::Format("(%.15f)", 1.0);

            TString selection = TString::Format("(%s) * (%s)", weights.Data(), basecut.Data());
            TString variable = TString::Format("Susy.MassLSP:Susy.MassGlu>>+%s", h_PN_MLSP_MSTop->GetName());
            fSamples[i].tree->Draw(variable, selection, "goff");

            TH2D *h_SMSEvents = (TH2D*) fSamples[i].file->Get("h_SMSEvents");
            h_SMSEvents->Rebin2D(5, 5);
            h_TN_MLSP_MSTop->Add(h_SMSEvents);

        } else if (fSamples[i].type == "mc") { // WARNING: checking statistical fluctuation in this pt bin!!!
            TString weights = TString::Format("(%.15f*pileUp.Weight)", weight);

            TString selection = TString::Format("(%s) * (%s)", weights.Data(), basecut.Data());
            TString variable = TString::Format("misc.MT2>>+%s", h_PMT2_Bkg->GetName());
            fSamples[i].tree->Draw(variable, selection, "goff");

            h_PN_Bkg->SetBinContent(1, h_PMT2_Bkg->GetBinContent(2));
            h_PN_Bkg->SetBinError(1, h_PMT2_Bkg->GetBinError(2));

        }
    }//i fsamples.size()

    h_Eff_MLSP_MSTop->Divide(h_PN_MLSP_MSTop, h_TN_MLSP_MSTop, 1., 1., "B");

    //TString xsecfile = "/home/fahim/scratch0/mt2top20/CMSSW_5_3_7_patch1/src/MT2_V02-03-02/MT2Analysis/Code/MT2AnalysisCode/MT2Code/src/referenceXSecs.root";
    TString xsecfile = "referenceXSecs.root";
    TH1* hXsec = (TH1*) TFile::Open(xsecfile)->Get("stop8TeV_NLONLL");
    TH1* hXsecUn = (TH1*) TFile::Open(xsecfile)->Get("stop8TeV_NLONLL_unc");
    for (int i = 1; i <= h_PN_MLSP_MSTop->GetNbinsX(); i++) {
        for (int j = 1; j <= h_PN_MLSP_MSTop->GetNbinsY(); j++) {
            double mstop = h_PN_MLSP_MSTop->GetXaxis()->GetBinLowEdge(i);
            int imstop = hXsec->GetXaxis()->FindBin(mstop);
            double xsec = hXsec->GetBinContent(imstop);
            double wght = xsec * fSamples[0].lumi / h_TN_MLSP_MSTop ->GetBinContent(i, j);
            if (h_Eff_MLSP_MSTop->GetBinContent(i, j) > 0) {
                h_PN_MLSP_MSTop->SetBinContent(i, j, wght * h_PN_MLSP_MSTop->GetBinContent(i, j));
                h_PN_MLSP_MSTop->SetBinError(i, j, wght * h_PN_MLSP_MSTop->GetBinError(i, j));
            }
        }//j
    }//i

    delete h_PMT2_Bkg;
    delete h_TN_MLSP_MSTop;

    fout->Write();
    fout->Close();
}

void MassPlotter::TEST_QCDEstimation(TString var, TString basecut, TString upper_cut, TString lower_cut,
        const int nbinsX, const double *binsX, double fit_min, double fit_max, double pessimistic_point, TString sout, int chs) {

    // === B O O K I N G ===
    TH2D *h_TEST_DPHI_MT2_data = new TH2D("TEST_DPHI_MT2_data", "", 100, 0, 400, 100, 0, TMath::Pi());
    h_TEST_DPHI_MT2_data->Sumw2();
    TH2D *h_TEST_DPHI_MT2_sub = new TH2D("TEST_DPHI_MT2_sub", "", 100, 0, 400, 100, 0, TMath::Pi());
    h_TEST_DPHI_MT2_sub->Sumw2();
    TH2D *h_TEST_DPHI_MT2_sms = new TH2D("TEST_DPHI_MT2_sms", "SUSY-like events", 100, 0, 400, 100, 0, TMath::Pi());
    h_TEST_DPHI_MT2_sms->Sumw2();
    TH2D *h_TEST_DPHI_MT2_ewk = new TH2D("TEST_DPHI_MT2_ewk", "", 100, 0, 400, 100, 0, TMath::Pi());
    h_TEST_DPHI_MT2_ewk->Sumw2();
    TH2D *h_TEST_DPHI_MT2_qcd = new TH2D("TEST_DPHI_MT2_qcd", "QCD events", 100, 0, 400, 100, 0, TMath::Pi());
    h_TEST_DPHI_MT2_qcd->Sumw2();

    // #####################
    TH1D *h_TEST_rt_MT2_sms = new TH1D("TEST_rt_MT2_sms", "", nbinsX, binsX);
    h_TEST_rt_MT2_sms->Sumw2();
    TH1D *h_TEST_up_MT2_sms = new TH1D("TEST_up_MT2_sms", "", nbinsX, binsX);
    h_TEST_up_MT2_sms->Sumw2();
    TH1D *h_TEST_dn_MT2_sms = new TH1D("TEST_dn_MT2_sms", "", nbinsX, binsX);
    h_TEST_dn_MT2_sms->Sumw2();
    TH1D *h_TEST_ex_MT2_sms = new TH1D("TEST_ex_MT2_sms", "", nbinsX, binsX);
    h_TEST_ex_MT2_sms->Sumw2();

    TH1D *h_TEST_rt_MT2_qcd = new TH1D("TEST_rt_MT2_qcd", "", nbinsX, binsX);
    h_TEST_rt_MT2_qcd->Sumw2();
    TH1D *h_TEST_up_MT2_qcd = new TH1D("TEST_up_MT2_qcd", "", nbinsX, binsX);
    h_TEST_up_MT2_qcd->Sumw2();
    TH1D *h_TEST_dn_MT2_qcd = new TH1D("TEST_dn_MT2_qcd", "", nbinsX, binsX);
    h_TEST_dn_MT2_qcd->Sumw2();
    TH1D *h_TEST_ex_MT2_qcd = new TH1D("TEST_ex_MT2_qcd", "", nbinsX, binsX);
    h_TEST_ex_MT2_qcd->Sumw2();

    TH1D *h_TEST_rt_MT2_data = new TH1D("TEST_rt_MT2_data", "", nbinsX, binsX);
    h_TEST_rt_MT2_data->Sumw2();
    TH1D *h_TEST_up_MT2_data = new TH1D("TEST_up_MT2_data", "", nbinsX, binsX);
    h_TEST_up_MT2_data->Sumw2();
    TH1D *h_TEST_dn_MT2_data = new TH1D("TEST_dn_MT2_data", "", nbinsX, binsX);
    h_TEST_dn_MT2_data->Sumw2();
    TH1D *h_TEST_ex_MT2_data = new TH1D("TEST_ex_MT2_data", "", nbinsX, binsX);
    h_TEST_ex_MT2_data->Sumw2();

    TH1D *h_TEST_rt_MT2_sub = new TH1D("TEST_rt_MT2_sub", "", nbinsX, binsX);
    h_TEST_rt_MT2_sub->Sumw2();
    TH1D *h_TEST_up_MT2_sub = new TH1D("TEST_up_MT2_sub", "", nbinsX, binsX);
    h_TEST_up_MT2_sub->Sumw2();
    TH1D *h_TEST_dn_MT2_sub = new TH1D("TEST_dn_MT2_sub", "", nbinsX, binsX);
    h_TEST_dn_MT2_sub->Sumw2();
    TH1D *h_TEST_ex_MT2_sub = new TH1D("TEST_ex_MT2_sub", "", nbinsX, binsX);
    h_TEST_ex_MT2_sub->Sumw2();

    TH1D *h_TEST_rt_MT2_ewk = new TH1D("TEST_rt_MT2_ewk", "", nbinsX, binsX);
    h_TEST_rt_MT2_ewk->Sumw2();
    TH1D *h_TEST_up_MT2_ewk = new TH1D("TEST_up_MT2_ewk", "", nbinsX, binsX);
    h_TEST_up_MT2_ewk->Sumw2();
    TH1D *h_TEST_dn_MT2_ewk = new TH1D("TEST_dn_MT2_ewk", "", nbinsX, binsX);
    h_TEST_dn_MT2_ewk->Sumw2();
    TH1D *h_TEST_ex_MT2_ewk = new TH1D("TEST_ex_MT2_ewk", "", nbinsX, binsX);
    h_TEST_ex_MT2_ewk->Sumw2();

    TH1D *h_TEST_upcm_MT2_qcd = new TH1D("TEST_upcm_MT2_qcd", "", nbinsX, binsX);
    h_TEST_upcm_MT2_qcd->Sumw2();
    TH1D *h_TEST_excm_MT2_qcd = new TH1D("TEST_excm_MT2_qcd", "", nbinsX, binsX);
    h_TEST_excm_MT2_qcd->Sumw2();
    TH1D *h_TEST_excm_MT2_sub = new TH1D("TEST_excm_MT2_sub", "", nbinsX, binsX);
    h_TEST_excm_MT2_sub->Sumw2();
    ...
    for (size_t i = 0; i < fSamples.size(); ++i) {
        ...
        TString sel_dn = TString::Format("(%s) * (%s && %s)", weights.Data(), basecut.Data(), lower_cut.Data());
        ...
        TString sel_dn_QCD = TString::Format("(%s) * (%s && %s && %s)", weights.Data(), removeJet.Data(), basecut.Data(), lower_cut.Data());
        ...
        if (fSamples[i].type == "susy") {
            variable = TString::Format("%s:misc.MT2>>+%s", var.Data(), h_TEST_DPHI_MT2_sms->GetName());
            int nev2d = fSamples[i].tree->Draw(variable, selection, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_up_MT2_sms->GetName());
            int nev1dup = fSamples[i].tree->Draw(variable, sel_up, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_dn_MT2_sms->GetName());
            int nev1ddn = fSamples[i].tree->Draw(variable, sel_dn, "goff");

        } else if (fSamples[i].type == "mc" && (
                fSamples[i].name == "QCD_Pt_120to170"
                || fSamples[i].name == "QCD_Pt_170to300"
                || fSamples[i].name == "QCD_Pt_300to470"
                || fSamples[i].name == "QCD_Pt_470to600"
                || fSamples[i].name == "QCD_Pt_600to800"
                || fSamples[i].name == "QCD_Pt_800to1000"
                || fSamples[i].name == "QCD_Pt_1000to1400"
                || fSamples[i].name == "QCD_Pt_1400to1800"
                || fSamples[i].name == "QCD_Pt_1800"
                || fSamples[i].name == "QCD"
                )) { // WARNING: checking statistical fluctuation in this pt bin!!!
            variable = TString::Format("%s:misc.MT2>>+%s", var.Data(), h_TEST_DPHI_MT2_qcd->GetName());
            int nev2d = fSamples[i].tree->Draw(variable, selection_QCD, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_up_MT2_qcd->GetName());
            int nev1dup = fSamples[i].tree->Draw(variable, sel_up_QCD, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_dn_MT2_qcd->GetName());
            int nev1ddn = fSamples[i].tree->Draw(variable, sel_dn_QCD, "goff");
        } else if (fSamples[i].type == "mc") { // WARNING: checking statistical fluctuation in this pt bin!!!
            variable = TString::Format("%s:misc.MT2>>+%s", var.Data(), h_TEST_DPHI_MT2_ewk->GetName());
            int nev2d = fSamples[i].tree->Draw(variable, selection, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_up_MT2_ewk->GetName());
            int nev1dup = fSamples[i].tree->Draw(variable, sel_up, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_dn_MT2_ewk->GetName());
            int nev1ddn = fSamples[i].tree->Draw(variable, sel_dn, "goff");
        } else if (fSamples[i].type == "data") { // WARNING: checking statistical fluctuation in this pt bin!!!
            variable = TString::Format("%s:misc.MT2>>+%s", var.Data(), h_TEST_DPHI_MT2_data->GetName());
            int nev2d = fSamples[i].tree->Draw(variable, selection, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_up_MT2_data->GetName());
            int nev1dup = fSamples[i].tree->Draw(variable, sel_up, "goff");
            variable = TString::Format("%s>>+%s", var2.Data(), h_TEST_dn_MT2_data->GetName());
            int nev1ddn = fSamples[i].tree->Draw(variable, sel_dn, "goff");
        }
    ...
    h_TEST_DPHI_MT2_sub->Add(h_TEST_DPHI_MT2_data, h_TEST_DPHI_MT2_ewk, 1.0, -1.0);
    h_TEST_up_MT2_sub->Add(h_TEST_up_MT2_data, h_TEST_up_MT2_ewk, 1.0, -1.0);
    h_TEST_dn_MT2_sub->Add(h_TEST_dn_MT2_data, h_TEST_dn_MT2_ewk, 1.0, -1.0);


    for (int i = 0; i <= nbinsX; i++) {
        if (h_TEST_dn_MT2_sub->GetBinContent(i) < 0) h_TEST_dn_MT2_sub->SetBinContent(i, 0.);
        if (h_TEST_up_MT2_sub->GetBinContent(i) < 0) h_TEST_up_MT2_sub->SetBinContent(i, 0.);
    }


    h_TEST_rt_MT2_qcd->Divide(h_TEST_up_MT2_qcd, h_TEST_dn_MT2_qcd, 1., 1., "B");
    h_TEST_rt_MT2_data->Divide(h_TEST_up_MT2_data, h_TEST_dn_MT2_data, 1., 1., "B");
    h_TEST_rt_MT2_sub->Divide(h_TEST_up_MT2_sub, h_TEST_dn_MT2_sub, 1., 1., "B");

    // F I T ===========
    TF1 *fqcd1 = new TF1("fqcd1", "exp([0]+[1]*x)", binsX[1], binsX[nbinsX]);
    fqcd1->SetLineColor(kGreen);

    std::stringstream stmp;
    stmp << "exp([0]+[1]*" << pessimistic_point << ")+exp([0]+[1]*x)";
    TF1 *fqcd2 = new TF1("fqcd2", stmp.str().c_str(), binsX[1], binsX[nbinsX]);
    fqcd2->SetLineColor(kBlue);

    TF1 *fqcd3 = new TF1("fqcd3", "exp([0]+[1]*x)+[2]", binsX[1], binsX[nbinsX]);
    fqcd3->SetLineColor(kRed);

    TF1 *fsub1 = new TF1("fsub1", "exp([0]+[1]*x)", binsX[1], binsX[nbinsX]);
    fsub1->SetLineColor(kGreen);

    TF1 *fsub2 = new TF1("fsub2", stmp.str().c_str(), binsX[1], binsX[nbinsX]);
    fsub2->SetLineColor(kBlue);
    TF1 *fsub2up = new TF1("fsub2up", stmp.str().c_str(), binsX[1], binsX[nbinsX]);
    TF1 *fsub2dn = new TF1("fsub2dn", stmp.str().c_str(), binsX[1], binsX[nbinsX]);
    fsub2up->SetLineColor(kBlue);
    fsub2up->SetLineStyle(2);
    fsub2dn->SetLineColor(kBlue);
    fsub2dn->SetLineStyle(2);

    h_TEST_rt_MT2_qcd->Fit("fqcd1", "0", "", fit_min, fit_max);

    fqcd2->SetParameter(0, fqcd1->GetParameter(0));
    fqcd2->SetParameter(1, fqcd1->GetParameter(1));

    h_TEST_rt_MT2_qcd->Fit("fqcd3", "0", "", fit_min, 2500);

    h_TEST_rt_MT2_sub->Fit("fsub1", "0", "", fit_min, fit_max);
    fsub2->SetParameter(0, fsub1->GetParameter(0));
    fsub2->SetParameter(1, fsub1->GetParameter(1));
    fsub2up->SetParameter(0, fsub1->GetParameter(0) + fsub1->GetParError(0));
    fsub2up->SetParameter(1, fsub1->GetParameter(1) + fsub1->GetParError(1));
    fsub2dn->SetParameter(0, fsub1->GetParameter(0) - fsub1->GetParError(0));
    fsub2dn->SetParameter(1, fsub1->GetParameter(1) - fsub1->GetParError(1));

    // === E X P E C T A T I O N ===
    for (int i = 1; i <= nbinsX + 1; i++) {

        double dsubdnX = h_TEST_dn_MT2_sub->GetBinCenter(i);
        double dsubdnY = h_TEST_dn_MT2_sub->GetBinContent(i);
        double dsubdnYErr = h_TEST_dn_MT2_sub->GetBinError(i);
        double dsubRto = fsub2->Eval(dsubdnX);
        double dsubRtoErr = fabs(fsub2up->Eval(dsubdnX) - fsub2dn->Eval(dsubdnX)) / 2;

        h_TEST_ex_MT2_sub->SetBinContent(i, dsubdnY * dsubRto);
        h_TEST_ex_MT2_sub->SetBinError(i, (dsubdnYErr * dsubRto + dsubdnY * dsubRtoErr));
    }

    h_TEST_ex_MT2_qcd = (TH1D*) h_TEST_dn_MT2_qcd->Clone(h_TEST_ex_MT2_qcd->GetName());
    h_TEST_ex_MT2_qcd->Multiply(fqcd2);

    // === C U M U L A T I V E ===    
    for (int i = 1; i <= nbinsX; i++) {
        double err;
        h_TEST_upcm_MT2_qcd->SetBinContent(i, h_TEST_up_MT2_qcd->IntegralAndError(i, nbinsX + 1, err));
        h_TEST_upcm_MT2_qcd->SetBinError(i, err);
        h_TEST_excm_MT2_qcd->SetBinContent(i, h_TEST_ex_MT2_qcd->IntegralAndError(i, nbinsX + 1, err));
        h_TEST_excm_MT2_qcd->SetBinError(i, err);
        h_TEST_excm_MT2_sub->SetBinContent(i, h_TEST_ex_MT2_sub->IntegralAndError(i, nbinsX + 1, err));
        h_TEST_excm_MT2_sub->SetBinError(i, err);
    }

    // === O U T - F I L E S ===
    TFile *fout = new TFile(sout, "CREATE");
    fout->cd();
    ...
    TH1D *hExSb = new TH1D("hExSb", "hExSb", nbinsX, binsX);
    TH1D *hExSbSt = new TH1D("hExSbSt", "hExSbSt", nbinsX, binsX);
    TH1D *hExSbSy = new TH1D("hExSbSy", "hExSbSy", nbinsX, binsX);

    for (int i = 1; i < nbinsX + 1; i++) {
        if (h_TEST_up_MT2_qcd->GetBinLowEdge(i) >= 125) {
            cout << "\t>> [" << h_TEST_up_MT2_qcd->GetBinLowEdge(i)
                    << "-" << h_TEST_up_MT2_qcd->GetBinLowEdge(i + 1) << ")"

                    << "\t" << h_TEST_up_MT2_qcd->GetBinContent(i)
                    << " +- " << h_TEST_up_MT2_qcd->GetBinError(i)

                    << "\t" << h_TEST_ex_MT2_sub->GetBinContent(i)
                    << " +- " << h_TEST_ex_MT2_sub->GetBinError(i)
                    << endl;

            hExSb->SetBinContent(i, h_TEST_ex_MT2_sub->GetBinContent(i));
            hExSbSt->SetBinContent(i, h_TEST_ex_MT2_sub->GetBinError(i));
        }
    }
    ...
    switch (chs) {
        ...
        {
            // ========== SYSTEMATICS ==========
            vector<double> vExSbSy;
            int const nExperiment = 1000;
            for (int i = 0; i < nExperiment; i++) {

                TRandom *r0 = new TRandom(rand() % 100 + 1); //seed in the range 1 to 100
                double a = r0->Gaus(fit_min, 0.1 * fit_min);
                double b = r0->Gaus(fit_max, 0.1 * fit_max);
                double c = r0->Gaus(pessimistic_point, 0.1 * pessimistic_point);

                stmp.str(" ");
                stmp << "exp([0]+[1]*" << c << ")+exp([0]+[1]*x)";
                TF1 *fsubSy = new TF1("fsubSy", stmp.str().c_str(), binsX[1], binsX[nbinsX]);
                h_TEST_rt_MT2_sub->Fit("fsubSy", "0", "", a, b);

                TH1D* hsubSy = (TH1D*) h_TEST_dn_MT2_sub->Clone("hsubSy");
                hsubSy->Multiply(fsubSy);

                for (int j = 0; j < nbinsX; j++) vExSbSy.push_back(hsubSy->GetBinContent(j + 1));
            ...
            for (int i = 0; i < nbinsX; i++) {
                double d[nExperiment] = {};
                for (int j = 0; j < nExperiment; j++) d[j] = vExSbSy[i + j * nbinsX];
                if (hExSbSy->GetBinLowEdge(i + 1) >= 125) hExSbSy->SetBinContent(i + 1, TMath::RMS(nExperiment, d));
            }
            break;
        }
        case 1:
        {
            ...
            // ===  ===
            TCanvas *can02 = new TCanvas("can02", "", 0, 0, 600, 500);
            can02->SetLogz();
            gPad->SetRightMargin(0.2);

            h_TEST_DPHI_MT2_qcd->GetXaxis()->SetTitle("M_{T2} [GeV/c]");
            h_TEST_DPHI_MT2_qcd->GetYaxis()->SetTitle("#Delta#phi_{min}");
            h_TEST_DPHI_MT2_qcd->GetZaxis()->SetRangeUser(1., 1000.);

            h_TEST_DPHI_MT2_qcd->Draw("colorBox");

            // ===  ===
            TCanvas *can03 = new TCanvas("can03", "", 0, 0, 600, 500);
            can03->SetLogz();
            gPad->SetRightMargin(0.2);

            h_TEST_DPHI_MT2_sms->GetXaxis()->SetTitle("M_{T2} [GeV/c]");
            h_TEST_DPHI_MT2_sms->GetYaxis()->SetTitle("#Delta#phi_{min}");
            h_TEST_DPHI_MT2_sms->GetZaxis()->SetRangeUser(0.001, .1);
            h_TEST_DPHI_MT2_sms->Draw("colorBox");

            // ===  ===
            TCanvas *can04 = new TCanvas("can04", "ratio qcd", 0, 0, 600, 500);
            can04->SetLogy(1);

            h_TEST_rt_MT2_qcd->GetXaxis()->SetTitle("M_{T2} [GeV/c]");
            h_TEST_rt_MT2_qcd->GetYaxis()->SetTitle("Ratio");
            h_TEST_rt_MT2_qcd->GetYaxis()->SetRangeUser(0.01, 100);
            h_TEST_rt_MT2_qcd->SetMinimum(0.0001);
            h_TEST_rt_MT2_qcd->SetMarkerSize(1.3);
            h_TEST_rt_MT2_qcd->SetMarkerStyle(24);
            h_TEST_rt_MT2_qcd->SetMarkerColor(kBlue);
            h_TEST_rt_MT2_qcd->SetTitle("QCD Events");
            h_TEST_rt_MT2_qcd->Draw("p");

            fqcd1->SetTitle("Exp(a - b M_{T2})");
            fqcd1->Draw("same");

            stmp.str(" ");
            stmp << "Exp(a - b M_{T2}) + c(" << pessimistic_point << "GeV)";
            fqcd2->SetTitle(stmp.str().c_str());
            fqcd2->Draw("same");

            fqcd3->SetTitle("Exp(a - b M_{T2}) + c");
            fqcd3->Draw("same");

            TLegend *leg04 = new TLegend(.55, .5, .85, .75);
            leg04->SetFillColor(0);
            leg04->AddEntry(h_TEST_rt_MT2_qcd);
            leg04->AddEntry(fqcd1);
            leg04->AddEntry(fqcd2);
            leg04->AddEntry(fqcd3);
            leg04->Draw("same");

            // ===  ===
            TCanvas *can05 = new TCanvas("can05", "ratio data", 0, 0, 600, 500);
            can05->SetLogy(1);

            h_TEST_rt_MT2_sub->GetXaxis()->SetTitle("M_{T2} [GeV/c]");
            h_TEST_rt_MT2_sub->GetYaxis()->SetTitle("Ratio");
            h_TEST_rt_MT2_sub->GetYaxis()->SetRangeUser(0.01, 100);
            h_TEST_rt_MT2_sub->SetMinimum(0.01);
            h_TEST_rt_MT2_sub->SetMarkerSize(1.3);
            h_TEST_rt_MT2_sub->SetMarkerStyle(20);
            h_TEST_rt_MT2_sub->SetMarkerColor(kBlue);
            h_TEST_rt_MT2_sub->SetTitle("data (non-QCD subtracted)");

            h_TEST_rt_MT2_sub->Draw("pE1");

            h_TEST_rt_MT2_data->SetMinimum(0.01);
            h_TEST_rt_MT2_data->SetMarkerSize(1.3);
            h_TEST_rt_MT2_data->SetMarkerStyle(22);
            h_TEST_rt_MT2_data->SetMarkerColor(kRed);
            h_TEST_rt_MT2_data->SetTitle("data");
            h_TEST_rt_MT2_data->Draw("pE1same");

            fsub1->SetTitle("Exp(a - b M_{T2})");
            fsub1->Draw("same");

            fsub2->SetTitle(stmp.str().c_str());
            fsub2->Draw("same");
            //    fsub2up->Draw("same");
            //    fsub2dn->Draw("same");

            TLine *l_sub_min = new TLine(fit_min, fsub2->GetYaxis()->GetXmin(), fit_min, fsub2->GetMaximum());
            l_sub_min->SetLineStyle(2);
            l_sub_min->Draw("same");
            TLine *l_sub_max = new TLine(fit_max, fsub2->GetYaxis()->GetXmin(), fit_max, fsub2->GetMaximum());
            l_sub_max->SetLineStyle(2);
            l_sub_max->Draw("same");

            TLegend *leg = new TLegend(.55, .5, .85, .75);
            leg->SetFillColor(0);
            leg->AddEntry(h_TEST_rt_MT2_data);
            leg->AddEntry(h_TEST_rt_MT2_sub);
            leg->AddEntry(fsub1);
            leg->AddEntry(fsub2);
            leg->Draw("same");
            ...
            // ===  ===
            TCanvas *can06 = new TCanvas("can06", "expected_observed", 0, 0, 600, 500);
            can06->SetLogy(1);


            h_TEST_up_MT2_qcd->GetYaxis()->SetRangeUser(1e-4, 5e4);
            h_TEST_up_MT2_qcd->SetMinimum(1e-4);
            h_TEST_up_MT2_qcd->SetMarkerSize(1.3);
            h_TEST_up_MT2_qcd->SetMarkerStyle(24);
            h_TEST_up_MT2_qcd->SetMarkerColor(kBlue);
            h_TEST_up_MT2_qcd->SetTitle("QCD MC-truth");
            h_TEST_up_MT2_qcd->GetXaxis()->SetTitle("M_{T2} [GeV/c]");
            h_TEST_up_MT2_qcd->GetYaxis()->SetTitle("Number of Events");

            h_TEST_up_MT2_qcd->Draw("p");

            h_TEST_ex_MT2_sub->SetMarkerSize(1.3);
            h_TEST_ex_MT2_sub->SetMarkerStyle(3);
            h_TEST_ex_MT2_sub->SetMarkerColor(kBlue);
            h_TEST_ex_MT2_sub->SetTitle("QCD Expected from data (non-QCD subtracted)");
            h_TEST_ex_MT2_sub->Draw("psame");

            TLegend *leg06 = new TLegend(.55, .5, .85, .75);
            leg06->SetFillColor(0);
            leg06->AddEntry(h_TEST_up_MT2_qcd);
            leg06->AddEntry(h_TEST_ex_MT2_sub);
            leg06->Draw("same");


            // ===  ===
            //            TCanvas *can07 = new TCanvas("can07", "cumulative expected_observed", 0, 0, 600, 500);
            //            can07->SetLogy(1);
            //
            //            h_TEST_up_MT2_data->Draw("TEXT45");
            //            h_TEST_up_MT2_ewk->Draw("sameTEXT45");
            //            h_TEST_up_MT2_sub->Draw("sameTEXT45");
            //            
            //            h_TEST_dn_MT2_data->Draw("sameTEXT45");
            //            h_TEST_dn_MT2_ewk->Draw("sameTEXT45");
            //            h_TEST_dn_MT2_sub->Draw("sameTEXT45");

            //h_TEST_upcm_MT2_qcd->SetMarkerSize(1.3);
            //h_TEST_upcm_MT2_qcd->SetMarkerSize(1.3);
            //h_TEST_upcm_MT2_qcd->SetMarkerStyle(24);
            //h_TEST_upcm_MT2_qcd->SetMarkerColor(kBlue);
            //h_TEST_upcm_MT2_qcd->Draw("p");

            //h_TEST_excm_MT2_sub->SetMarkerSize(1.3);
            //h_TEST_excm_MT2_sub->SetMarkerStyle(3);
            //h_TEST_excm_MT2_sub->SetMarkerColor(kBlue);
            //h_TEST_excm_MT2_sub->Draw("psame");
...
void MassPlotter::ABCD_MT2(TString var, TString basecut, TString upper_cut, TString lower_cut, const int nbins, const double *bins, double fit_min, double fit_max) {
    TH2D *h_ABCD_MT2_qcd = new TH2D("ABCD_MT2__qcd", "", 100, 0, 600, 180, 0, TMath::Pi());
    h_ABCD_MT2_qcd ->Sumw2();
    TH2D *h_ABCD_MT2_susy = new TH2D("ABCD_MT2__susy", "", 100, 0, 600, 180, 0, TMath::Pi());
    h_ABCD_MT2_susy ->Sumw2();
    TH1D *h_ABCD_lower_y_band_susy = new TH1D("ABCD_lower_y_band__susy", "", nbins, bins);
    h_ABCD_lower_y_band_susy->Sumw2();
    TH1D *h_ABCD_upper_y_band_susy = new TH1D("ABCD_upper_y_band__susy", "", nbins, bins);
    h_ABCD_upper_y_band_susy->Sumw2();
    TH1D *h_ABCD_lower_y_band_data = new TH1D("ABCD_lower_y_band__data", "", nbins, bins);
    h_ABCD_lower_y_band_data->Sumw2();
    TH1D *h_ABCD_upper_y_band_data = new TH1D("ABCD_upper_y_band__data", "", nbins, bins);
    h_ABCD_upper_y_band_data->Sumw2();
    TH1D *h_ABCD_lower_y_band_qcd = new TH1D("ABCD_lower_y_band__qcd", "", nbins, bins);
    h_ABCD_lower_y_band_qcd ->Sumw2();
    TH1D *h_ABCD_upper_y_band_qcd = new TH1D("ABCD_upper_y_band__qcd", "", nbins, bins);
    h_ABCD_upper_y_band_qcd ->Sumw2();
    TH1D *h_ABCD_lower_y_band_mc = new TH1D("ABCD_lower_y_band__mc", "", nbins, bins);
    h_ABCD_lower_y_band_mc ->Sumw2();
    TH1D *h_ABCD_upper_y_band_mc = new TH1D("ABCD_upper_y_band__mc", "", nbins, bins);
    h_ABCD_upper_y_band_mc ->Sumw2();
    TH1D *h_ABCD_lower_y_band_sub = new TH1D("ABCD_lower_y_band__sub", "", nbins, bins);
    h_ABCD_lower_y_band_sub ->Sumw2();
    TH1D *h_ABCD_upper_y_band_sub = new TH1D("ABCD_upper_y_band__sub", "", nbins, bins);
    h_ABCD_upper_y_band_sub ->Sumw2();
    TH1D *ratio_data = new TH1D("ratio__data", "", nbins, bins);
    ratio_data ->Sumw2();
    TH1D *ratio_qcd = new TH1D("ratio__qcd", "", nbins, bins);
    ratio_qcd ->Sumw2();
    TH1D *ratio_sub = new TH1D("ratio__sub", "", nbins, bins);
    ratio_sub ->Sumw2();
    ...
    h_ABCD_lower_y_band_data->SetLineColor(1);
    h_ABCD_upper_y_band_data->SetLineColor(2);
    h_ABCD_lower_y_band_qcd ->SetLineColor(1);
    h_ABCD_upper_y_band_qcd ->SetLineColor(2);
    h_ABCD_lower_y_band_sub ->SetLineColor(1);
    h_ABCD_upper_y_band_sub ->SetLineColor(2);
    h_ABCD_lower_y_band_sub ->SetLineStyle(3);
    h_ABCD_upper_y_band_sub ->SetLineStyle(3);
    ratio_sub->SetLineStyle(3);
    ratio_qcd ->SetMarkerStyle(24);
    ratio_qcd ->SetMarkerSize(0.8);
    ratio_data->SetMarkerStyle(24);
    ratio_data->SetMarkerSize(0.8);
    ratio_sub ->SetMarkerStyle(24);
    ratio_sub ->SetMarkerSize(0.8);
    ratio_qcd ->SetXTitle("M_{T2}");
    ratio_qcd ->SetYTitle("ratio");
    ratio_sub ->SetXTitle("M_{T2}");
    ratio_sub ->SetYTitle("ratio");
    ...
    TF1 *f_qcd1 = new TF1("f_qcd1", "expo(0)", bins[0], bins[nbins]);
    f_qcd1->SetLineColor(8);
    TF1 *f_sub1 = new TF1("f_sub1", "expo(0)", bins[0], bins[nbins]);
    f_sub1->SetLineColor(8);
    TF1 *f_qcd2 = new TF1("f_qcd2", "exp([0]+200.*[1])+expo(0)", bins[0], bins[nbins]);
    f_qcd2->SetLineColor(9);
    TF1 *f_sub2 = new TF1("f_sub2", "exp([0]+200.*[1])+expo(0)", bins[0], bins[nbins]);
    f_sub2->SetLineColor(8);
    TF1 *f_qcd3 = new TF1("f_qcd3", "expo(0)+[2]", bins[0], bins[nbins]);
    f_qcd3->SetLineColor(50);
    ...
    f_qcd3->Draw("same");
    TLine *l_qcd_min = new TLine(fit_min, f_qcd1->GetYaxis()->GetXmin(), fit_min, f_qcd1->GetMaximum());
    l_qcd_min->SetLineStyle(2);
    l_qcd_min->Draw("same");
    TLine *l_qcd_max = new TLine(fit_max, f_qcd1->GetYaxis()->GetXmin(), fit_max, f_qcd1->GetMaximum());
    l_qcd_max->SetLineStyle(2);
    l_qcd_max->Draw("same");
    ...
    f_sub2->Draw("same");
    TLine *l_sub_min = new TLine(fit_min, f_sub1->GetYaxis()->GetXmin(), fit_min, f_sub1->GetMaximum());
    l_sub_min->SetLineStyle(2);
    l_sub_min->Draw("same");
    TLine *l_sub_max = new TLine(fit_max, f_sub1->GetYaxis()->GetXmin(), fit_max, f_sub1->GetMaximum());
    l_sub_max->SetLineStyle(2);
    l_sub_max->Draw("same");
    TLegend *leg = new TLegend(.55, .5, .85, .75);
    leg->SetFillColor(0);
...
void MassPlotter::PrintABCDPredictions(TString var, TString basecut, TString upper_cut, TString lower_cut, TF1* func_qcd, TF1* func_sub, TF1* func_qcd_model) {
    ...
    TH1D *h_pred_lower_y_band_data = new TH1D("pred_lower_y_band__data", "", nbins, min, max);
    h_pred_lower_y_band_data->Sumw2();
    TH1D *h_pred_upper_y_band_data = new TH1D("pred_upper_y_band__data", "", nbins, min, max);
    h_pred_upper_y_band_data->Sumw2();
    TH1D *h_pred_lower_y_band_qcd = new TH1D("pred_lower_y_band__qcd", "", nbins, min, max);
    h_pred_lower_y_band_qcd ->Sumw2();
    TH1D *h_pred_upper_y_band_qcd = new TH1D("pred_upper_y_band__qcd", "", nbins, min, max);
    h_pred_upper_y_band_qcd ->Sumw2();
    TH1D *h_pred_lower_y_band_mc = new TH1D("pred_lower_y_band__mc", "", nbins, min, max);
    h_pred_lower_y_band_mc ->Sumw2();
    TH1D *h_pred_upper_y_band_mc = new TH1D("pred_upper_y_band__mc", "", nbins, min, max);
    h_pred_upper_y_band_mc ->Sumw2();
    TH1D *h_pred_lower_y_band_sub = new TH1D("pred_lower_y_band__sub", "", nbins, min, max);
    h_pred_lower_y_band_sub ->Sumw2();
    TH1D *h_pred_upper_y_band_sub = new TH1D("pred_upper_y_band__sub", "", nbins, min, max);
    h_pred_upper_y_band_sub ->Sumw2();
    TH1D *h_pred_lower_y_band_susy = new TH1D("pred_lower_y_band__susy", "", nbins, min, max);
    h_pred_lower_y_band_susy->Sumw2();
    TH1D *h_pred_upper_y_band_susy = new TH1D("pred_upper_y_band__susy", "", nbins, min, max);
    h_pred_upper_y_band_susy->Sumw2();
    ...
    TH1D *h_pred_lower_y_band_data_2 = (TH1D*) h_pred_lower_y_band_data->Clone("h_pred_lower_y_band_data_2");
    h_pred_lower_y_band_data_2 ->Sumw2();
    TH1D *h_pred_lower_y_band_qcd_2 = (TH1D*) h_pred_lower_y_band_qcd ->Clone("h_pred_lower_y_band_qcd_2");
    h_pred_lower_y_band_qcd_2 ->Sumw2();
    TH1D *h_pred_lower_y_band_sub_2 = (TH1D*) h_pred_lower_y_band_sub ->Clone("h_pred_lower_y_band_sub_2");
    h_pred_lower_y_band_sub_2 ->Sumw2();
    TH1D *h_pred_lower_y_band_data_c = (TH1D*) h_pred_lower_y_band_data->Clone("h_pred_lower_y_band_data_c");
    h_pred_lower_y_band_data_c ->Sumw2();
    TH1D *h_pred_lower_y_band_qcd_c = (TH1D*) h_pred_lower_y_band_qcd ->Clone("h_pred_lower_y_band_qcd_c");
    h_pred_lower_y_band_qcd_c ->Sumw2();
    TH1D *h_pred_lower_y_band_sub_c = (TH1D*) h_pred_lower_y_band_sub ->Clone("h_pred_lower_y_band_sub_c");
    h_pred_lower_y_band_sub_c ->Sumw2();
    TH1D *h_pred_lower_y_band_susy_c = (TH1D*) h_pred_lower_y_band_susy->Clone("h_pred_lower_y_band_susy_c");
    h_pred_lower_y_band_susy_c ->Sumw2();
    TH1D *h_pred_lower_y_band_data_2_c = (TH1D*) h_pred_lower_y_band_data->Clone("h_pred_lower_y_band_data_2_c");
    h_pred_lower_y_band_data_2_c ->Sumw2();
    TH1D *h_pred_lower_y_band_qcd_2_c = (TH1D*) h_pred_lower_y_band_qcd ->Clone("h_pred_lower_y_band_qcd_2_c");
    h_pred_lower_y_band_qcd_2_c ->Sumw2();
    TH1D *h_pred_lower_y_band_sub_2_c = (TH1D*) h_pred_lower_y_band_sub ->Clone("h_pred_lower_y_band_sub_2_c");
    h_pred_lower_y_band_sub_2_c ->Sumw2();
    TH1D *h_pred_lower_y_band_qcd_model = (TH1D*) h_pred_lower_y_band_qcd ->Clone("h_pred_lower_y_band_qcd_model");
    h_pred_lower_y_band_qcd_model->Sumw2();
...
void MassPlotter::vs(){ 
         ...
         TH2F *VSPTvsMET = new TH2F("VSPTvsMET", "VSPTvsMET", 100, 0,1000, 100,0,1000);
         TH2F *MT2vsMET = new TH2F("MT2vsMET", "MT2vsMET", 100, 0,1000, 100,0,1000);
         TH2F *VSPTvsNVTX = new TH2F("VSPTvsNVTX", "VSPTvsNVTX", 100, 0,1000, 100,0,1000);
    ...
    Sample.tree->SetBranchAddress("MT2tree", &fMT2tree);
    float Weight = Sample.xsection * Sample.kfact * Sample.lumi / (Sample.nevents * Sample.PU_avg_weight );
     Weight = Weight * fMT2tree->pileUp.Weight;
    ...
    for (Long64_t j=0; j<n;j++) {
     ...
     if (!(fMT2tree->misc.MT2             >  125                               &&
           (fMT2tree->misc.ProcessID!=10 || (fMT2tree->Susy.MassGlu >= 350 && fMT2tree->Susy.MassGlu < 355 &&
            fMT2tree->Susy.MassLSP >= 50 && fMT2tree->Susy.MassLSP < 55))         &&
         fMT2tree->NBJetsCSVT>=1                                               &&
            fMT2tree->misc.MET>=30                                                &&
            fMT2tree->misc.MinMetJetDPhi4 >0.3                                    &&
            fMT2tree->misc.Vectorsumpt < 70                                      &&
            fMT2tree->NJetsIDLoose40  >=4                                              &&
            (fMT2tree->NEles==0  || ele[0].lv.Pt()<5)                                  &&
            (fMT2tree->NMuons==0 || muo[0].lv.Pt()<5)                                  &&
            fMT2tree->misc.Jet0Pass ==1                                                &&
            fMT2tree->misc.Jet1Pass ==1                                               &&
            fMT2tree->misc.PassJetID ==1                                              &&
   ...
   //   VSPTvsMET->Fill(fMT2tree->misc.MET, fMT2tree->misc.Vectorsumpt,Weight);
     MT2vsMET->Fill(fMT2tree->misc.MET, fMT2tree->misc.MT2,Weight);
     VSPTvsNVTX->Fill(fMT2tree->misc.MET, fMT2tree->misc.Vectorsumpt,Weight);
...
/*
   TCanvas *MyC = new TCanvas("MyC", "MyC");
    ...
    MyC->cd(1);
    VSPTvsMET->SetYTitle("VSPT");
    VSPTvsMET->SetXTitle("MET");
    VSPTvsMET->SetTitle("Top");
    VSPTvsMET->Draw("COLZ");

    TCanvas *MyC1 = new TCanvas("MyC1", "MyC1");

    MyC1->cd(1);
     MT2vsMET->SetYTitle("MT2");
     MT2vsMET->SetXTitle("MET");
     MT2vsMET->SetTitle("Top");
     MT2vsMET->Draw("COLZ");

}
/*/

void MassPlotter::MT2forVSPT(){
  
  TH1F *MT2forVSPT0to20     = new TH1F("MT2forVSPT0to20", "MT2forVSPT0to20", 100, 0,200);
  TH1F *MT2forVSPT20to30    = new TH1F("MT2forVSPT20to30", "MT2forVSPT20to30", 100, 0,200);
  TH1F *MT2forVSPT30to40    = new TH1F("MT2forVSPT30to40", "MT2forVSPT30to40", 100, 0,200);
  TH1F *MT2forVSPT40to50    = new TH1F("MT2forVSPT40to50", "MT2forVSPT40to50", 100, 0,200);
  TH1F *MT2forVSPT50to70    = new TH1F("MT2forVSPT50to70", "MT2forVSPT50to70", 100, 0,200);
  TH1F *MT2forVSPT70to100   = new TH1F("MT2forVSPT70to100", "MT2forVSPT70to100", 100, 0,200);
  TH1F *MT2forVSPT100to1000 = new TH1F("MT2forVSPT100to1000", "MT2forVSPT100to1000", 100, 0,200);
   ...
   /*   if ((!(fMT2tree->misc.MT2             >  125 )))                              &&
            ...
            fMT2tree->misc.MT2 >= 125                                           &&
           (fMT2tree->misc.ProcessID!=10 || (fMT2tree->Susy.MassGlu >= 350 && Susy.MassGlu < 355 &&
            fMT2tree->Susy.MassLSP >= 50 && fMT2tree->Susy.MassLSP < 55))         &&
            fMT2tree->NBJetsCSVT>=1                                               &&
            fMT2tree->misc.MET>=30                                                &&
  //        fMT2tree->misc.MinMetJetDPhi4 >0.3                                    &&
...
(fMT2tree->NJetsIDLoose40  >=7 && fMT2tree->jet[6].lv.Pt() >= 55.0 )))))))))
	...
	continue;
*/
      if(fMT2tree->misc.Vectorsumpt > 0 && fMT2tree->misc.Vectorsumpt <= 20)
      MT2forVSPT0to20->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 20 && fMT2tree->misc.Vectorsumpt <= 30)  
      MT2forVSPT20to30->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 30 && fMT2tree->misc.Vectorsumpt <= 40)      
      MT2forVSPT30to40->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 40 && fMT2tree->misc.Vectorsumpt <= 50)       
      MT2forVSPT40to50->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 50 && fMT2tree->misc.Vectorsumpt <= 70) 
      MT2forVSPT50to70->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 70 && fMT2tree->misc.Vectorsumpt <= 100)      
      MT2forVSPT70to100->Fill(fMT2tree->misc.MT2, weight);


      if(fMT2tree->misc.Vectorsumpt > 100 && fMT2tree->misc.Vectorsumpt <= 1000)  
      MT2forVSPT100to1000->Fill(fMT2tree->misc.MT2, weight);
...
if (j%50000 == 0){cout << j <<endl;}
    ...
    MT2forVSPT0to20->SetName("QCD 170-300");
    MT2forVSPT0to20->SetXTitle("MT2");
  //  MT2forVSPT0to20->SetYTitle("QCD 170-300");

    MT2forVSPT0to20->Scale(1/MT2forVSPT0to20->Integral());
    MT2forVSPT0to20->Draw();
    MT2forVSPT0to20->SetLineColor(1);

    MT2forVSPT20to30->Scale(1/MT2forVSPT20to30->Integral());
    MT2forVSPT20to30->Draw("same");
    MT2forVSPT20to30->SetLineColor(2);

    MT2forVSPT30to40->Scale(1/MT2forVSPT30to40->Integral());
    MT2forVSPT30to40->Draw("same");
    MT2forVSPT30to40->SetLineColor(3);

    MT2forVSPT40to50->Scale(1/MT2forVSPT40to50->Integral());
    MT2forVSPT40to50->Draw("same");
    MT2forVSPT40to50->SetLineColor(4);

    MT2forVSPT50to70->Scale(1/MT2forVSPT50to70->Integral());
    MT2forVSPT50to70->Draw("same");
    MT2forVSPT50to70->SetLineColor(5);

    MT2forVSPT70to100->Scale(1/MT2forVSPT70to100->Integral());
    MT2forVSPT70to100->Draw("same");
    MT2forVSPT70to100->SetLineColor(6);

    MT2forVSPT100to1000->Scale(1/MT2forVSPT100to1000->Integral()); 
    MT2forVSPT100to1000->Draw("same");
    MT2forVSPT100to1000->SetLineColor(7);

TLegend* leg = new TLegend(0.35,0.6,0.85,0.90);
  leg->SetFillStyle (0);
  leg->SetFillColor (0);
  leg->SetBorderSize(0);
  leg->AddEntry(MT2forVSPT0to20,     "0<Vectorsumpt<20"               , "PL");
  leg->AddEntry(MT2forVSPT20to30,    "20<Vectorsumpt<30"              , "F");
  leg->AddEntry(MT2forVSPT30to40,    "30<Vectorsumpt<40"              , "F");
  leg->AddEntry(MT2forVSPT40to50,    "40<Vectorsumpt<50"              , "F");
  leg->AddEntry(MT2forVSPT50to70,    "50<Vectorsumpt<70"              , "F");
  leg->AddEntry(MT2forVSPT70to100,   "70<Vectorsumpt<100"             , "F");
  leg->AddEntry(MT2forVSPT100to1000, "100<Vectorsumpt<1000"           , "F");
...
void MassPlotter::vs(){ 
         ...
         TH2F *VSPTvsMET = new TH2F("VSPTvsMET", "VSPTvsMET", 100, 0,1000, 100,0,1000);
         TH2F *MT2vsMET = new TH2F("MT2vsMET", "MT2vsMET", 100, 0,1000, 100,0,1000);
         TH2F *VSPTvsNVTX = new TH2F("VSPTvsNVTX", "VSPTvsNVTX", 100, 0,100, 1000,0,1000);
 //        TProfile *hprof  = new TProfile("hprof","Profile of VSPTvsNVTX",100,0,100,0,100,"e");
//         TH1F *VSPT= new TH1F("VSPT", "VSPT", 100, 0,1000);
//         TH1F *NVTX= new TH1F("NVTX", "NVTX", 10, 0,50);
    ...
    for (Long64_t j=0; j<n;j++) {
      ...
      Sample.tree->GetEntry(j); 
  //  cout << "Pile-up Weight is:" << fMT2tree->pileUp.Weight <<endl;
 ...
 if (!fMT2tree->misc.isData )
  ...
  //          fMT2tree->misc.Vectorsumpt < 70                                      &&
   ...
   //  VSPTvsMET->Fill(fMT2tree->misc.MET, fMT2tree->misc.Vectorsumpt,Weight);
  //   MT2vsMET->Fill(fMT2tree->misc.MET, fMT2tree->misc.MT2,Weight);
 //    VSPT->Fill(fMT2tree->misc.Vectorsumpt,Weight);
       VSPTvsNVTX->Fill(fMT2tree->pileUp.NVertices, fMT2tree->misc.Vectorsumpt, weight);
  //     hprof->Fill(fMT2tree->pileUp.NVertices,fMT2tree->misc.Vectorsumpt, weight);
...
/*
 ...
 //        NVTX->Fill(fMT2tree->pileUp.NVertices); 
	}
//	fMT2tree->pileUp.NVertices=nvertex;
//      NVXT->Fill(fMT2tree->pileUp.NVertices,Weight);   
...
*/
    ...
    MyC1->cd(1);
     VSPTvsNVTX->SetYTitle("VSPT");
     VSPTvsNVTX->SetXTitle("NVertices");
     VSPTvsNVTX->SetTitle("");
     VSPTvsNVTX->Draw("COLZ");



//     TCanvas *MyC2 = new TCanvas("MyC2","Profile histogram example",200,10,700,500);
//     MyC2->cd(1);
//     hprof->Draw();

}

void MassPlotter::vs1(){ 
 
         TH2F *PUvsNVX = new TH2F("PUvsNVX", "PUvsNVX", 100, 0,100, 10,0,100);
    ...
    for (Long64_t j=0; j<n;j++) {
      ...
      Sample.tree->GetEntry(j); 
       ...
       PUvsNVX->Fill(fMT2tree->pileUp.NVertices, fMT2tree->pileUp.Weight, weight);
    ...
    MyC1->cd(1);
     PUvsNVX->SetYTitle("Pile-Up");
     PUvsNVX->SetXTitle("NVertices");
     PUvsNVX->SetTitle("");
     PUvsNVX->Draw("COLZ");
