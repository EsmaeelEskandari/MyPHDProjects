...
//                fMT2tree->ele[i].LooseEleMVA   = fMT2tree->ele[i].IsGoodLooseEleMVA(); 
//                fMT2tree->ele[i].TightEleMVA   = fMT2tree->ele[i].IsGoodTightEleMVA();
...
//electron identification based on MVA
/*bool MT2Analysis::IsGoodLooseEleMVA() {
                 
                  if  (fTR->ElEta[fElecs[i]] < 0.8 && fTR->ElPt [fElecs[i]] <= 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.925)  return true;

                  if  (0.8 <= fTR->ElEta[fElecs[i]] < 1.479 && fTR->ElPt [fElecs[i]] <= 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.915) return true;

                  if  (1.479 <= fTR->ElPt [fElecs[i]] && fTR->ElPt [fElecs[i]] <= 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.965) return true;

                  if  (fTR->ElEta[fElecs[i]] < 0.8 && fTR->ElPt [fElecs[i]] > 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.905)  return true;

                  if  (0.8 <= fTR->ElEta[fElecs[i]] < 1.479 && fTR->ElPt [fElecs[i]] > 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.955) return true;

                  if  (1.479 <= fTR->ElPt [fElecs[i]] && fTR->ElPt [fElecs[i]] > 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.975) return true;

}
bool MT2Analysis::IsGoodTightEleMVA() {
                  ...
                  if  (1.479 <= fTR->ElPt [fElecs[i]] && fTR->ElPt [fElecs[i]] > 20)
                       if (fTR->fTeIDMva[fElecs[i]] > 0.985) return true;
...
void MT2Analysis::Initialize_JetCorrectionUncertainty(){
	string PF  ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+ (fisCHSJets?"_Uncertainty_AK5PFchs.txt":"_Uncertainty_AK5PF.txt");
	string Calo="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_Uncertainty_AK5Calo.txt";
...
void MT2Analysis::Initialize_JetEnergyCorrection(){
	string Calo_L2  ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L2Relative_AK5Calo.txt";
	string Calo_L3  ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L3Absolute_AK5Calo.txt";
	string Calo_RES ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L2L3Residual_AK5Calo.txt";
	string PF_L1    ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+(fisCHSJets?"_L1FastJet_AK5PFchs.txt"    :"_L1FastJet_AK5PF.txt");
	string PF_L2    ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+(fisCHSJets?"_L2Relative_AK5PFchs.txt"   :"_L2Relative_AK5PF.txt");
	string PF_L3    ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+(fisCHSJets?"_L3Absolute_AK5PFchs.txt"   :"_L3Absolute_AK5PF.txt");
	string PF_RES   ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+(fisCHSJets?"_L2L3Residual_AK5PFchs.txt" :"_L2L3Residual_AK5PF.txt");
	string rawPF_L1 ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L1FastJet_AK5PF.txt";
	string rawPF_L2 ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L2Relative_AK5PF.txt";
	string rawPF_L3 ="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L3Absolute_AK5PF.txt";
	string rawPF_RES="/public/MT2Top/JetEnergyCorrection/"+fJEC+"/"+fJEC+"_L2L3Residual_AK5PF.txt";
...
VSPTvsNVTX->Fill(fMT2tree->pileUp.NVertices, fMT2tree->misc.Vectorsumpt);
