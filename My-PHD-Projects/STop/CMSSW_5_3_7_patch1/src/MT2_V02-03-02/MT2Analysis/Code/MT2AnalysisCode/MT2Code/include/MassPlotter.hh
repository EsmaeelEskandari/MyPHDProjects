...
//	void PrintZllEfficiency(int sample_index, bool data, std::string lept, Long64_t nevents, double lower_mass, double upper_mass, bool pileup_weight);
	void PrintWEfficiency(int sample_index ,TString process, std::string lept, Long64_t nevents, bool includeTaus);
        ...
        void TEST_Counting_MT2(TString basecut);
        
        void TEST_QCDEstimation(TString var="misc.MinMetJetDPhi", TString basecut="misc.HBHENoiseFlag == 1",
		      TString upper_cut="misc.MinMetJetDPhi>0.3", TString lower_cut="misc.MinMetJetDPhi<0.2",
		      const int nbinsX=gNMT2bins, const double *binsX=gMT2bins, 
                      double fit_min=40., double fit_max=100., double pessimistic_point = 250, TString sout = "qcdEstimation.root", int chs = 0);
  ...
  void TauContamination(int sample_index, Long64_t nevents, int flag, TString cuts, TString trigger, int signalContamination);
