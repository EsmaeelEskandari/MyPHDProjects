...
struct leptonicW{
	...
	TH1D * hnW_leptveto_scaled(){
		...
		if(firstime_leptveto == true) for (i = 1; i <= 9 ; i++ ) { 	
...
//	TH1::SetDefaultSumw2();
	...
	TH1D * f = new TH1D("f","f",2,0,2);	
        ...
        f->SetBinContent(1,0.80198);
        f->SetBinContent(2,0.798236);
        f->SetBinError(1,0.0240849);
        f->SetBinError(2,0.0282576);
...
void getEfficiencies(Long64_t nevents){
	...
	if(fMakeEfficienciesPresel){
		...
		else        tA->init("samples/samplesMineSmearedJet.dat");	
...
*/
	...
	tA->makePlot("misc.MT2"                        , ele_cuts,  -4, -1, -11 ,  trigger, "M_{T2}"     , /*60*/9, 0, 1500           ,   false,   true , true,   true,  true,  false , 1);
	tA->makePlot("misc.MT2"                        , muo_cuts,  -4, -1, -13 ,  trigger, "M_{T2}"     , /*60*/9, 0, 1500           ,   false,   true , true,   true,  true,  false , 1);
	...
	zbg_muo[4] = zbg_m->GetBinContent(5);
	zbg_muo[5] = zbg_m->GetBinContent(6);
	zbg_muo[6] = zbg_m->GetBinContent(7);
	zbg_muo[7] = zbg_m->GetBinContent(8);
	zbg_muo[8] = zbg_m->GetBinContent(9);
	...
	zbg_ele[4] = zbg_e->GetBinContent(5);
	zbg_ele[5] = zbg_e->GetBinContent(6);
	zbg_ele[6] = zbg_e->GetBinContent(7);
	zbg_ele[7] = zbg_e->GetBinContent(8);
	zbg_ele[8] = zbg_e->GetBinContent(9);
	...
	qcdbg_muo[4] = qcdbg_m->GetBinContent(5);
	qcdbg_muo[5] = qcdbg_m->GetBinContent(6);
	qcdbg_muo[6] = qcdbg_m->GetBinContent(7);
	qcdbg_muo[7] = qcdbg_m->GetBinContent(8);
	qcdbg_muo[8] = qcdbg_m->GetBinContent(9);
	...
	qcdbg_ele[4] = qcdbg_e->GetBinContent(5);
	qcdbg_ele[5] = qcdbg_e->GetBinContent(6);
	qcdbg_ele[6] = qcdbg_e->GetBinContent(7);
	qcdbg_ele[7] = qcdbg_e->GetBinContent(8);
	qcdbg_ele[8] = qcdbg_e->GetBinContent(9);

	// delete pointer
	delete tA;
/*
	cout<<"--->>>"<<" Wele.QCD_bg: "<<Wele.QCD_bg<<" from integral: "<<(Wele.qcdbg)->Integral()<<endl;
	cout<<"--->>>"<<" Wele.Z_bg: "<<Wele.Z_bg<<" from integral: "<<(Wele.zbg)->Integral()<<endl;
...
double get_n_events(string process, bool background, bool leptveto, int sample_index = -1){
	...
	else        path = "/dataLOCAL/MT2Top/RootFiles/NewTreesSmearedJets2ndTry/";
	...
	else        path_data="/dataLOCAL/MT2Top/RootFiles/NewTrees20Invfb1stTry/";
...
void LostLeptonInMT2Bins(){
	...
	{
		cout<<"		-> bin "<<i<<" -------------------------------------------------------------"<<endl;
		double lostlepton = ((Wele.hdata())->GetBinContent(i)-(Wele.hnW_bg_scaled())->GetBinContent(i)-zbg_ele[i]-qcdbg_ele[i])*elefactor; 
		cout<<"			-> electrons: "<<lostlepton<<" compared to MC-truth: "<<(Wele.hnW_leptveto_scaled())->GetBinContent(i)<<" +- "<<Wele.binByBinMCTruthError[i]<<endl;
		cout<<"				      "<<" +- "<<sqrt((Wele.hdata())->GetBinContent(i))*elefactor<<" (stat) "<<endl;
		cout<<"				      "<<" +- "<<sqrt(pow((((Wele.hdata())->GetBinContent(i)-(Wele.hnW_bg_scaled())->GetBinContent(i)-zbg_ele[i]-qcdbg_ele[i])*eleerror),2)+pow(((Wele.hnW_bg_scaled())->GetBinContent(i)+zbg_ele[i]+qcdbg_ele[i])*elefactor,2))<<" (sys) "<<endl;
		lostlepton = ((Wmuo.hdata())->GetBinContent(i) - (Wmuo.hnW_bg_scaled())->GetBinContent(i)-zbg_muo[i]-qcdbg_muo[i])*muofactor; 
		cout<<"			-> muons: "<<lostlepton<<" compared to MC-truth: "<<(Wmuo.hnW_leptveto_scaled())->GetBinContent(i)<<" +- "<<Wmuo.binByBinMCTruthError[i]<<endl;
		cout<<"				      "<<" +- "<<sqrt((Wmuo.hdata())->GetBinContent(i))*muofactor<<" (stat) "<<endl;
		cout<<"				      "<<" +- "<<sqrt(pow((((Wmuo.hdata())->GetBinContent(i)-(Wmuo.hnW_bg_scaled())->GetBinContent(i)-zbg_muo[i]-qcdbg_muo[i])*muoerror),2)+pow(((Wmuo.hnW_bg_scaled())->GetBinContent(i)+zbg_muo[i]+qcdbg_muo[i])*muofactor,2))<<" (sys) "<<endl;
	}
}
