...
// ________________________________________________________________________
map<int, double> MassPlotter::calcWeight(){

	map<int, double> mymap;
 
        for(size_t i = 0; i < fSamples.size(); ++i){
        sample Sample = fSamples[i];
	double weight = Sample.PU_avg_weight;

	mymap.insert(make_pair(i, weight));
	}

	return mymap;
}

void MassPlotter::PrintContent(Long64_t nevents, bool includeTaus){

        for(size_t i = 0; i < fSamples.size(); ++i){
        sample Sample = fSamples[i];
	cout << "loading printcontent for: " << Sample.name << endl;
	cout << "		sname      " << Sample.sname << endl;
	if (!((Sample.sname == "Top") || (Sample.sname == "Wtolnu"))) continue;
	//if (!(Sample.name == "TTbar")) continue;
	...
	int nLepVeto_muo=0;
		...
		// NJetsIDLoose
		if(fMT2tree->NJetsIDLoose40                 <4     ) continue;  
		if(fMT2tree->misc.PassJetID               ==0     ) continue;
		// MinMetJetDPhi
		if(fMT2tree->misc.MinMetJetDPhi4 <=0.3 )       continue;
		// nbjets
		if(fMT2tree->misc.Vectorsumpt             >=70     )  continue;
		if(fMT2tree->NBJetsCSVT == 0     )  continue;
		// MT2
		if(fMT2tree->misc.MT2             <=125     )  continue;
		// lepton veto
		//if(!(fMT2tree->NEles==1 && fMT2tree->NMuons==0 && fMT2tree->ele[0].MT<100)) continue;
		//if(!(fMT2tree->NEles==0 && fMT2tree->NMuons==1 && fMT2tree->muo[0].MT<100)) continue;
		if(!(fMT2tree->NEles==0 && fMT2tree->NMuons==0)) continue;
		// Trigger
        	if(!(((fMT2tree->NJetsIDLoose40  >=4 && fMT2tree->jet[3].lv.Pt() >= 100.0) || (fMT2tree->NJetsIDLoose40  >=6 && fMT2tree->jet[5].lv.Pt() >= 65.0))||((fMT2tree->NJetsIDLoose40  >=5 && fMT2tree->jet[4].lv.Pt() >= 85.0) || (fMT2tree->NJetsIDLoose40  >=7 && fMT2tree->jet[6].lv.Pt() >= 55.0)))) continue;
	...
	cout << "+++ Proccessing event " << jentry << endl;
        ...
        for(int i = 0; i < fMT2tree->NGenLepts; i++){
          if((abs(fMT2tree->genlept[i].ID) == 11) && (fMT2tree->genlept[i].MID == fMT2tree->genlept[j].ID) && (abs(fMT2tree->genlept[i].GMID)==24)) tau_ele = 1;
          if((abs(fMT2tree->genlept[i].ID) == 13) && (fMT2tree->genlept[i].MID == fMT2tree->genlept[j].ID) && (abs(fMT2tree->genlept[i].GMID)==24)) tau_muo = 1;
        }
...
if(Sample.sname == "Top") {
        cout<<" TopDecayMode "<<fMT2tree->TopDecayMode()<<endl;
        cout<<" TopDecayModeResult(11) "<<fMT2tree->TopDecayModeResult(11)<<endl;
        cout<<" TopDecayModeResult(13) "<<fMT2tree->TopDecayModeResult(13)<<endl;
}
if(Sample.sname == "Wtolnu") {
	cout<<" W -> e nu "<<fMT2tree->GenLeptFromW(11, 0 , 1000,true)<<endl;
	cout<<" W -> m nu "<<fMT2tree->GenLeptFromW(13, 0 , 1000,true)<<endl;
} 
	for (int i = 0; i < 120; i++) if (i == fMT2tree->TopDecayMode()) n[i]++;
	if (Sample.sname == "Wtolnu" && fMT2tree->GenLeptFromW(11, 0 , 1000,true)==1 && fMT2tree->GenLeptFromW(13, 0 , 1000,true)==0) nLepVeto_ele++;
	if (Sample.sname == "Wtolnu" && fMT2tree->GenLeptFromW(11, 0 , 1000,true)==0 && fMT2tree->GenLeptFromW(13, 0 , 1000,true)==1) nLepVeto_muo++;
	if (Sample.sname == "Top" && fMT2tree->TopDecayModeResult(11)) nLepVeto_ele++; 
	if (Sample.sname == "Top" && fMT2tree->TopDecayModeResult(13)) nLepVeto_muo++; 
...
void MassPlotter::PrintWEfficiency(int sample_index ,TString process,  std::string lept, Long64_t nevents, bool includeTaus){
	...
	cout << "printing W efficiency: \n"
	     << "::: " << Sample.name << " , lept " << lept << " process " << process << endl;	
	std::cout << setfill('=') << std::setw(70) << "" << std::endl;
	//std::cout << setfill('-') << std::setw(70) << "" << std::endl;

	enum counters_t { count_begin, all=count_begin, presel, NJetsIDLoose , MinMetJetDPhi, nbjets, MT2, Trigger,  count_end };
	Monitor counters[count_end];
	TString lablx[count_end] = {"all events", "presel", "NJetsIDLoose" , "MinMetJetDPhi", "nbjets", "MT2", "Trigger"};
	...
	for (Long64_t jentry=0; jentry<min(nentries, nevents);jentry++) {
		...
		if(process=="W"){
			...
			if(lept=="ele" && fMT2tree->NEles==1 && fMT2tree->NMuons==0 && fMT2tree->GenLeptFromW(11, 0, 1000,includeTaus)==true && fMT2tree->ele[0].MT<100)  leptfound =true;
			if(lept=="muo" && fMT2tree->NMuons==1&& fMT2tree->NEles== 0 && fMT2tree->GenLeptFromW(13, 0, 1000,includeTaus)==true && fMT2tree->muo[0].MT<100)  leptfound =true;
		}else if(process=="Top"){ 
			...
			if(lept=="ele" && fMT2tree->NEles==1 && fMT2tree->NMuons==0 && eventgood &&fMT2tree->ele[0].MT<100)             leptfound =true;
			if(lept=="muo" && fMT2tree->NMuons==1&& fMT2tree->NEles== 0 && eventgood &&fMT2tree->muo[0].MT<100)             leptfound =true;
	...
	//if (HadTau == 0) {cout<<" leptonic ----------"<<endl;
	//if (tau_ele == 1) cout<<"		tau_ele "<<endl; 
	//if (tau_muo == 1) cout<<"	        tau_muo "<<endl;}
	if (HadTau == 1) //hadDecayTau = true; 
	cout<<" hadronic !!!!!!!!!!"<<endl;
}
*/
		}

		//if (hadDecayTau == true) continue;

		Double_t weight = fMT2tree->pileUp.Weight * fMT2tree->SFWeight.BTagCSV40ge1;
		//Double_t weight = fMT2tree->pileUp.Weight;
		...
		// MinMetJetDPhi
        	if(fMT2tree->misc.MinMetJetDPhi4 <0 )       continue;
		//if(fMT2tree->misc.MinMetJetDPhi4 <=0.3 )       continue;
		...
		if(fMT2tree->misc.Vectorsumpt             >=70     )  continue;
		if(fMT2tree->NBJetsCSVT == 0     )  continue;
		if(eventgood)   counters[nbjets].fill("nbjets", weight);
		if(acceptance)  counters[nbjets].fill("acceptance", weight);
		if(leptfound)   counters[nbjets].fill(to_measure, weight);
		...
		// Trigger
        	//if(!(((fMT2tree->NJetsIDLoose40  >=4 && fMT2tree->jet[3].lv.Pt() >= 100.0) || (fMT2tree->NJetsIDLoose40  >=6 && fMT2tree->jet[5].lv.Pt() >= 65.0))||((fMT2tree->NJetsIDLoose40  >=5 && fMT2tree->jet[4].lv.Pt() >= 85.0) || (fMT2tree->NJetsIDLoose40  >=7 && fMT2tree->jet[6].lv.Pt() >= 55.0)))) continue;
        	//if(!(((fMT2tree->NJetsIDLoose40  >=4 && fMT2tree->jet[3].lv.Pt() >= 100.0) || (fMT2tree->NJetsIDLoose40  >=6 && fMT2tree->jet[5].lv.Pt() >= 65.0))||((fMT2tree->NJetsIDLoose40  >=5 && fMT2tree->jet[4].lv.Pt() >= 85.0)))) continue;
        	if(!(((fMT2tree->NJetsIDLoose40  >=4 && fMT2tree->jet[3].lv.Pt() >= 80.0))||((fMT2tree->NJetsIDLoose40  >=5 && fMT2tree->jet[4].lv.Pt() >= 65.0)))) continue;
		if(eventgood)   counters[Trigger].fill("Trigger", weight);
		if(acceptance)  counters[Trigger].fill("acceptance", weight);
		if(leptfound)   counters[Trigger].fill(to_measure, weight);
	}
...
void MassPlotter::MakePlot(std::vector<sample> Samples, TString var, TString cuts, int njets, int nbjets, int nleps, TString HLT,
  ...
  //double bins[10]={0,25,50,75,100,125,150,200,300,450}; 
  //  double bins[5]={125,150,200,250,1500}; 
...
void MassPlotter::MakePlot(std::vector<sample> Samples, TString var, TString maincuts, TString basecuts, int njets, int nbjets, int nleps, TString HLT,
 ...
 //	TString nBJets = "NBJets40CSVM";    // nbjets = -10  --> >=0 b-tags
	TString nBJets = "NBJetsCSVT";    // nbjets = -10  --> >=0 b-tags //..it seems that NBJets40CSVM is harder than NBJetsCSVT!! 
	//saeid
	nBJets += nbjets < 0 ? ">=" : "==";
	...
	for(size_t i = 0; i < Samples.size(); ++i){
		...
		if(fVerbose>2) cout << fixed << "MakePlot: looping over " << Samples[i].sname << "-----------------------------------" <<  endl;
	...
	if (composited){
	  ...
	  /*                      << "	QCD k-factor [0,50]:      " << (h_composited[6]->Integral(0,50)-h_composited[1]->Integral(0,50)-h_composited[2]->Integral(0,50)-h_composited[3]->Integral(0,50))/h_composited[0]->Integral(0,50)  << endl
	                        << "	QCD k-factor [0,60]:      " << (h_composited[6]->Integral(0,60)-h_composited[1]->Integral(0,60)-h_composited[2]->Integral(0,60)-h_composited[3]->Integral(0,60))/h_composited[0]->Integral(0,60)  << endl
	                        << "	QCD k-factor [0,70]:      " << (h_composited[6]->Integral(0,70)-h_composited[1]->Integral(0,70)-h_composited[2]->Integral(0,70)-h_composited[3]->Integral(0,70))/h_composited[0]->Integral(0,70)  << endl;*/
	...
	while( IN.getline(buffer, 200, '\n') ){
		...
		if( !strcmp(buffer, "SAMPLE")){
			...
			// DON'T DO THAT AT HOME !!!!
                        ...
                        if ( s.type == "susy" ){//&& s.file.Contains("T2tt")){//saeid
			...
			if(fVerbose > 0){
				cout << " ---- " << endl;
				cout << fixed << "  New sample added: " << s.name << endl;
