...
//  TString outF_name = "histos_ZInv-MT2_-1_9999-Jet40-HT_800-1Mu_pt10_4450pb_"+B;
  TString outF_name = "histos_ZInv-MT2_-1_9999-Jet40-HT_700-1Mu_pt10_4450pb_"+B;
  ...
  for(int i=0; i< nf; i++){
    ...
    */
    ...
    infile->GetObject("Lept_pt_"+names[i]+"_leptSel", Lept_pt_leptSel[i] );
    ...
    infile->GetObject("Lept_eta_"+names[i]+"_leptSel", Lept_eta_leptSel[i] );
  ...
  if(B=="1b"){
    ...
    */
    ...
    c[nc++] = new TCanvas(outF_name+"-MET_bSel",outF_name+"-MET_bSel" );  
    gPad->SetLogy();
