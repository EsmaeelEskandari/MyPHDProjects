...
struct leptonicW{
	...
	//static const double Wnevents2       =  4807610;
	static const double Wnevents2       =  4931110;//nr of events in WJetsToLNu_HT_250To300 were changed since when? i don't know!
	...
	static const double lumi           = 17432.5;
...
TH1D* findingRLL(){
	...
	TH1D * f = new TH1D("f","f",2,0,2);	
        f->SetBinContent(1,0.83063);
        f->SetBinContent(2,0.850654);
        f->SetBinError(1,0.0147118);
        f->SetBinError(2,0.0159006);
...
void run_LostLeptonSmearedJetMETNewBTagParked(){
...
void getEfficiencies(Long64_t nevents){
	...
	if(fMakeEfficienciesPresel){
		...
		else        tA->init("samples/samplesMineSmearedJetMETNewBTagParked.dat");	
...
*/
	...
	std::ostringstream cutStream;
        cutStream << " "
            ...
            <<"(((NJetsIDLoose40  >=4 && jet[3].lv.Pt() >= 80.0))"<<"||"
            <<"((NJetsIDLoose40  >=5 && jet[4].lv.Pt() >= 65.0)))";
...
*/
	...
	tA->makePlot("misc.MT2"                        , ele_cuts,  -4, -1, -11 ,  trigger, "M_{T2}"     , /*60*/4, 125/*0*/, 1500           ,   false,   true , true,   true,  true,  true , 1);
	tA->makePlot("misc.MT2"                        , muo_cuts,  -4, -1, -13 ,  trigger, "M_{T2}"     , /*60*/4, 125/*0*/, 1500           ,   false,   true , true,   true,  true,  true , 1);
...
void makePrediction(){
	...
	Wele.nData                     = get_n_events("ele_data"           , false, false, 26);
	Wmuo.nData                     = get_n_events("muo_data"           , false, false, 26);
	Wele.nSusy                     = get_n_events("ele_susy"           , false, false, 33);
	Wmuo.nSusy                     = get_n_events("muo_susy"           , false, false, 33);
	cout<<"*** nSUSY: "<<0.109621278*Wele.nSusy<<endl;
...
double get_n_events(string process, bool background, bool leptveto, int sample_index = -1){
	...
	else        path_data="/dataLOCAL/MT2Top/RootFiles/ParkedSmearedJetMETNewBTag/";

	if(process == "ele"){
		if (sample_index == 9) chain   ->Add(path+"WJetsToLNu-HT-200To250-8TeV-madgraph-Summer12-DR53X-PU-S10-START53-V7C-v1-2_parkedsmall.root");
		if (sample_index == 10) chain   ->Add(path+"WJetsToLNu-HT-250To300-8TeV-madgraph-v2-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if (sample_index == 11) chain   ->Add(path+"WJetsToLNu-HT-300To400-8TeV-madgraph-v2-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if (sample_index == 12) chain   ->Add(path+"WJetsToLNu-HT-400ToInf-8TeV-madgraph-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		...
		chain   ->Add(path+"TTJets-MassiveBinDECAY-TuneZ2star-8TeV-madgraph-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		...
		if (sample_index == 20) chain   ->Add(path+"T-s-channel-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if (sample_index == 21) chain   ->Add(path+"Tbar-s-channel-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if( background)basecut +="TopDecayModeResult(11)==0 &&";
		if(!leptveto)  basecut +="NEles==1 && NMuons==0 && ele[0].MT<100&&";
		else           basecut +="(NEles==0)&&(NMuons==0)&& TopDecayModeResult(11)==1 &&";
	} else if ( process == "eleTop_t"){
		if (sample_index == 22) chain   ->Add(path+"T-t-channel-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v3_parkedsmall.root");
		if (sample_index == 23) chain   ->Add(path+"Tbar-t-channel-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if( background)basecut +="TopDecayModeResult(11)==0 &&";
		if(!leptveto)  basecut +="NEles==1 && NMuons==0 && ele[0].MT<100&&";
		else           basecut +="(NEles==0)&&(NMuons==0) && TopDecayModeResult(11)==1 &&";
	} else if ( process == "eleTop_tW"){
		(sample_index == 24) chain   ->Add(path+"T-tW-channel-DR-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		(sample_index == 25) chain   ->Add(path+"Tbar-tW-channel-DR-TuneZ2star-8TeV-powheg-tauola-Summer12-DR53X-PU-S10-START53-V7A-v1_parkedsmall.root");
		if( background)basecut +="TopDecayModeResult(11)==0 &&";
		if(!leptveto)  basecut +="NEles==1 && NMuons==0 && ele[0].MT<100&&";
		else           basecut +="(NEles==0)&&(NMuons==0) && TopDecayModeResult(11)==1 &&";
	}else if(process == "muo"){
		...
		chain   ->Add(path+"WJetsToLNu-HT-250To300-8TeV-madgraph_parkedsmall.root");
		...
		chain   ->Add(path_data+"MultiJet1Parked-Run2012B-05Nov2012-v2_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012C-part1-05Nov2012-v2_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012C-part2-05Nov2012-v2-FirstHalf_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012C-part2-05Nov2012-v2-SecondHalf_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012D-part1-10Dec2012-v1-FirstHalf_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012D-part1-10Dec2012-v1-SecondHalf_parkedsmall.root");
		chain   ->Add(path_data+"MultiJet1Parked-Run2012D-part2-17Jan2013-v1_parkedsmall.root");
	...
	} else if ( process == "ele_susy"){
                chain   ->Add(path+"SMS-T2tt-mStop-150to350-mLSP-0to250-8TeV-Pythia6Z-Summer12-START52-V9-FSIM-v1-2_parkedsmall.root");
                basecut +="NMuons==0 && NEles==1 && ele[0].MT<100 && ";
        } else if ( process == "muo_susy"){
                chain   ->Add(path+"SMS-T2tt-mStop-150to350-mLSP-0to250-8TeV-Pythia6Z-Summer12-START52-V9-FSIM-v1-2_parkedsmall.root");
                basecut +="NMuons==1 && NEles==0 && muo[0].MT<100 && ";
        } else {return -1;}
	if(process == "ele_susy" || process == "muo_susy") basecut  += "Susy.MassGlu >= 350 && Susy.MassGlu < 355 && Susy.MassLSP >= 50 && Susy.MassLSP < 55 && ";   
	basecut  += "misc.MET>30 && ";
	...
	//basecut  += "misc.MinMetJetDPhi4 >0.3 &&";// cut on dPhi is relaxed for Lost Lepton estimation, applied on Wed 10 April
        ...
        basecut  += "(((NJetsIDLoose40  >=4 && jet[3].lv.Pt() >= 80.0)) || ((NJetsIDLoose40  >=5 && jet[4].lv.Pt() >= 65.0))) &&";
	// run on data only, corrected on 21 Jan
        ...
        basecut  += "((((trigger.HLT_QuadJet50_v1 ==1) ||";
        basecut  += "(trigger.HLT_QuadJet50_v2 ==1) ||";
        basecut  += "(trigger.HLT_QuadJet50_v3 ==1) ||";
        basecut  += "(trigger.HLT_QuadJet50_v5 ==1)) &&";
        basecut  += " ((NJetsIDLoose40  >=4 && jet[3].lv.Pt() >= 80.0) || ";// open pran
        basecut  +=" (NJetsIDLoose40  >=5 &&";
        basecut  +=" jet[4].lv.Pt() >= 65.0 ))))))";
	...
	TString selection;
	if(process == "ele_data" || process == "muo_data" || process == "ele_susy" || process == "muo_susy"){
...
void InclusiveEstimation(){
	...
	double sigCont = (Wele.nData+0.109621278*Wele.nSusy-Wele.bg())*elefactor;
	double stat_sc = sqrt(Wele.nData+0.109621278*Wele.nSusy)*elefactor;
	double system_sc = sqrt(pow(((Wele.nData+0.109621278*Wele.nSusy-Wele.bg())*eleerror),2)+pow(Wele.bg()*elefactor,2));
	cout<<"         >>>>>>>>> Signal Contamination "<<sigCont<<" +- "<<stat_sc<<" +- "<<system_sc<<endl;
	cout<<"*** muons ***"<<endl;
	...
	cout<<"		>>>>>>>>> MC prediction "<<mcpred<<endl;
	sigCont = (Wmuo.nData+0.109621278*Wmuo.nSusy-Wmuo.bg())*muofactor;
	stat_sc = sqrt(Wmuo.nData+0.109621278*Wmuo.nSusy)*muofactor;
	system_sc = sqrt(pow(((Wmuo.nData+0.109621278*Wmuo.nSusy-Wmuo.bg())*muoerror),2)+pow(Wmuo.bg()*muofactor,2));
	cout<<"         >>>>>>>>> Signal Contamination "<<sigCont<<" +- "<<stat_sc<<" +- "<<system_sc<<endl;
}
void LostLeptonInMT2Bins(){
	...
	{
		cout<<"		-> bin "<<i<<" -------------------------------------------------------------"<<endl;
		double lostlepton = ((Wele.hdata())->GetBinContent(i)-(Wele.hnW_bg_scaled())->GetBinContent(i)-zbg_ele[i-1]-qcdbg_ele[i-1])*elefactor; 
		...
		cout<<"				      "<<" +- "<<sqrt(pow((((Wele.hdata())->GetBinContent(i)-(Wele.hnW_bg_scaled())->GetBinContent(i)-zbg_ele[i-1]-qcdbg_ele[i-1])*eleerror),2)+pow(((Wele.hnW_bg_scaled())->GetBinContent(i)+zbg_ele[i-1]+qcdbg_ele[i-1])*elefactor,2))<<" (sys) "<<endl;
