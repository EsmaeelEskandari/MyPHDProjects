...
my $VERSION = sprintf "%d.%03d", q$Revision: 231335 $ =~ /(\d+)/g;
my $verbose;
...
my $artType = 0;
my @artTypes = ("Particle Physics - Experiment", "Nuclear Physics - Experiment", "Detectors and Experimental Techniques");
...
if ($help)
{
    ...
    "\n >>> makeManifest: pulls references to figures from a TDR log file and copies the actual figures
            ...
            Options/Parameters:\n
              ...
              - artType: set the article description to (0) 'Particle Physics - Experiment', (1) 'Nuclear Physics - Experiment', or (2) 'Detectors and Experimental Techniques'. Defaults to $artType.\n
...
#
   ...
   # \RCS$Revision: 231335 $
   # \RCS$Date: 2014-03-13 13:59:37 +0330 (Thu, 13 Mar 2014) $
   # \RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/utils/trunk/general/makeManifest.pl $
   # \RCS$Id: makeManifest.pl 231335 2014-03-13 10:29:37Z alverson $
...
#      &insert_datafield("859",$record,'cds.support@cern.ch');
      ...
      my $tag65017 = XML::Twig::Elt->new(datafield=>{tag=>"65017", ind1=>" ", ind2=>" "});
      &insert_subfield("2",$tag65017,"SzGeCERN");
      &insert_subfield("a",$tag65017,@artTypes[$artType]);
      $tag65017->paste('last_child',$record);
...
#---------- pull out the name/number
            ...
            $figPath = undef;
            my $line = $_;
            $line =~ s/\s+$//; # now occasionally has trailing white space
